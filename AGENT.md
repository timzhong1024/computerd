...
1. 用真实 runtime port 替换当前内存/fixture control plane。
2. 保持 `computer` 为主模型，不把 runtime helper 反向抬升成顶层对象。
3. 把 Web 代码从当前单文件状态继续收紧为更清晰的 transport/use-case/view-model/ui 边界。
4. 继续保持 MCP / HTTP / Web 共用同一套契约，而不是各自扩字段。
