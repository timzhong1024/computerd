...
export function App() {
  ...
  async function handleCreateComputer(event: React.FormEvent<HTMLFormElement>) {
...
async function getJson<T>(url: string, parser: (value: unknown) => T) {
...
async function postJson<T>(url: string, payload: unknown, parser: (value: unknown) => T) {
  const response = await fetch(url, {
    method: "POST",
    headers: {
